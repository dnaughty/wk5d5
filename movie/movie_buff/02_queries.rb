def eighties_b_movies
  ...
  #Movie.where("yr LIKE 198_ AND score BETWEEN 3 AND 5").select(:id, :title, :yr, :score)
...
def vanity_projects
  ...
  Movie.joins(:actors).where("movies.director_id = castings.actor_id").select(:id, :name, :title,) 
